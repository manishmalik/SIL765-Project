...
csv_files = ['np_experiment.csv', 'background_experiment.csv', 'foreground_experiment.csv']

# Create a color map for each file
colors = ['red', 'green', 'blue']  # specify colors for each file
...
ax.set_xlabel('Load Time Relative')
ax.set_ylabel('Rank')
ax.legend()

plt.savefig('site_isolation.png')
plt.show()